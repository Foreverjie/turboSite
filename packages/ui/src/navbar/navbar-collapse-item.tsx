...
const NavbarCollapseItem = forwardRef<NavbarCollapseItemProps, 'li'>(
  (props, ref) => {
    const {
      children,
      className,
    ...
    const index = useMemo(() => {
      if (collapseContext?.items) {
        return collapseContext.items?.findIndex(
          (item: any) => item?.props?.children === children,
        )
      }
    ...
    const itemTransitions = useMemo(() => {
      return {
        transitionDelay:
          transitionDelay ?? collapseContext?.transitionDelay ?? 0,
    ...
    const defaultDelay = useMemo(
      () =>
        collapseContext.items && index > -1
          ? (index / collapseContext.items.length) * 0.5 * 1000 +
            itemTransitions.transitionDelay
          : 0.1,
    ...
    const itemStyle = useMemo(() => {
      const timeDelay =
        isVisible && !collapseContext.hasScrolled ? defaultDelay : 0
