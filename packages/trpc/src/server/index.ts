...
import { BaseTRPCContext, TRPCConfig, WebTRPCContext } from '../types'
...
export function createTRPCFactory<TContext extends BaseTRPCContext = BaseTRPCContext>(
  ...
  return {
    ...
    // Add helper methods for creating common middleware
    createRequireUserProcedure: () => {
      const requireUserMiddleware = t.middleware(({ next, ctx }) => {
        if (!ctx.user) {
          throw new TRPCError({
            code: 'UNAUTHORIZED',
            message: 'You need to Sign In first.',
      ...
      return t.procedure.use(requireUserMiddleware)
    },
    createRestrictToProcedure: (allowedRoles: string[]) => {
      const restrictMiddleware = t.middleware(({ next, ctx }) => {
        if (!ctx.user) {
          throw new TRPCError({
            code: 'UNAUTHORIZED',
            message: 'You need to Sign In first.',
        ...
        const userRoles = ctx.user.roles || (ctx.user.role ? [ctx.user.role] : [])
      ...
      return t.procedure.use(restrictMiddleware)
    },
  }
}

/**
 * Alternative factory for when you need more control over the tRPC instance
 */
export function createCustomTRPCFactory<TContext = any>(
  trpcInstance: ReturnType<typeof initTRPC.context<TContext>>
) {
  const authMiddleware = trpcInstance.middleware(({ next, ctx }: any) => {
    if (!ctx.user) {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'You need to Sign In first.',
  ...
  return {
    t: trpcInstance,
    router: trpcInstance.router,
    middleware: trpcInstance.middleware,
    mergeRouters: trpcInstance.mergeRouters,
    publicProcedure: trpcInstance.procedure,
    protectedProcedure: trpcInstance.procedure.use(authMiddleware),
    createRoleProcedure: (roles: string[]) => {
      const roleMiddleware = trpcInstance.middleware(({ next, ctx }: any) => {
        if (!ctx.user) {
          throw new TRPCError({
            code: 'UNAUTHORIZED',
            message: 'You need to Sign In first.',
      ...
      return trpcInstance.procedure.use(roleMiddleware)
    },
  }
}
